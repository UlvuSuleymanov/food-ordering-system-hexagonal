...
public class Order extends AggregateRoot<OrderId> {
    ...
    public void initCancel(List<String> failureMessages){
        ...
        updateFailureMessages(failureMessages.stream().filter(s -> !s.isEmpty()).toList());
    ...
    public void cancel(List<String> failureMessages){
        if(!(orderStatus==OrderStatus.CANCELLING || orderStatus==OrderStatus.PENDING))
            throw new OrderDomainException("Order is not correct status for cancel operation");
        orderStatus=OrderStatus.CANCELLED;
    ...
    private void validateItemsPrice() {
        ...
        if (!price.equals(orderItemsTotal))
            throw new OrderDomainException("Total price: " + price.getAmount()
                    + " not equal to Order items total: " + orderItemsTotal.getAmount());

    }

    public void validateItemPrice(OrderItem orderItem) {
        if (!orderItem.isPriceValid())
            throw new OrderDomainException("Orter item price: " + orderItem.getPrice().getAmount() +
